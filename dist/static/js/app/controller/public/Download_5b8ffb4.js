...
define('js/app/controller/public/Download', ['js/app/controller/base', 'js/app/util/ajax', 'js/app/interface/GeneralCtr', 'js/app/interface/UserCtr'], function (base, Ajax, GeneralCtr, UserCtr) {
	...
	function setHtml() {
		base.showLoading();
		$("title").html(base.getText('下载橙wallet', lang));
		...
		$(".section .first").html(base.getText('1.第一次打开橙wallet的时候会弹出如下框。', lang));
		...
		$.when(getListCountry(), getAndroidUrl(), getIosUrl()).then(function () {
			base.hideLoading();
	...
	function getAndroidUrl() {
		return GeneralCtr.getSysConfigKey("h5_download_android").then(function (data) {
			androidUpdateUrl = replaceChannelName('http://app.thachain.org/appDownload/release/THA_1.6.1_{channel_name}.apk');
		}, function () {
	...
	function getIosUrl() {
		return GeneralCtr.getSysConfigKey("h5_download_ios").then(function (data) {
			iosUpdateUrl = replaceChannelName('http://app.thachain.org/appDownload/release/THA_1.6.1_{channel_name}.apk');
		}, function () {
			base.showMsg(base.getText("获取下载地址失败", lang));
		});
	}
